...
(defclass ldom nil
  ((list :initarg :list :accessor ldom-list)
   (val :initarg :val :accessor ldom-val)
   (prec :initarg :prec)
   (posc :initarg :posc)
   (finc :initarg :finc)
   (pos0 :initarg :pos0 :accessor ldom-pos0)
   (pos1 :initarg :pos1 :accessor ldom-pos1)
...
(defun ldom-skip-whitespace ()
  (while (looking-at-p "[ \n\t]")
    (forward-char 1)))

(defun ldom-read-form-1 (hash)
  (catch 'return
    (let ((stack (list (list 'list)))
	  (domstack (list (ldom :pos0 (point-marker)))))
      (while (not (eobp))
	(cond
	 ((looking-at-p ";")
	  (ldom-skip-comment))
	 ((looking-at-p "[ \t\n]")
	  (ldom-skip-whitespace))
	 ((looking-at-p "(")
	  (push (list 'list) stack)
	  (push (ldom :pos0 (point-marker)) domstack)
	  (forward-char))
	 ((looking-at-p "\"")
	 ...
	 ((looking-at-p ")")
	  ...
	  (let* ((list (pop stack))
	    ...
	    (setf (ldom-pos1 dom) pos1)
	    (setf (ldom-list dom) (caar stack))
	    (setf (ldom-val dom) (eval (ldom-list dom) t))
	    (puthash (ldom-list dom) ldom hash)
	    (put-text-property pos0 pos1 'ldom dom))
	  (when (= (length stack) 1)
...
(defun lisp-insertion-points (string)
  (let (points)
    (with-temp-buffer
      (insert string)
      (emacs-lisp-mode)
	...
	(cond ((looking-at-p "[[(]")
	       ;;(push (cons (1- (point)) (1- (point))) points)
	       (forward-char))
	      ((looking-at-p "[])]")
	       (forward-char))
	      ((looking-at-p ";")
	      ...
	      ((looking-at "[ \t\n]+")
	       (let* ((len (- (match-end 0) (match-beginning 0)))
		      (point (cons (1- (point)) (+ -1 (point) len))))
		 (unless (looking-back ")")
		   (push point points))
		 (forward-char len)))
	      ((looking-at-p "\n")
...
(defun lisp-string-indent (string)
  (with-temp-buffer
    (insert string)
    (emacs-lisp-mode)
    (indent-region (point-min) (point-max))
...
(defun lisp-string-badness (string)
  (with-temp-buffer
    (insert string)
    (lisp-badness (point-min) (point-max))))

(defun lisp-find-candidates (string)
  (let ((points (lisp-insertion-points string))
	(cands (list (cons (lisp-string-badness string) string))))
    (dolist (point points)
      (let ((newstr (lisp-string-indent
		     (concat (substring string 0 (car point))
			     "\n"
			     (substring string (cdr point))))))
	(push (cons (lisp-string-badness newstr) newstr)
	      cands)))
    (dolist (point points)
      (let ((newstr (lisp-string-indent
		     (concat (substring string 0 (car point))
	...
	(push (cons (lisp-string-badness newstr) newstr)
	      cands)))
    (setq cands (sort cands (lambda (a b) (< (car a) (car b)))))
    (cdar cands)))
  ...
  (save-excursion
	...
	(catch 'next
	  (while (not (bobp))
	    ...
	    (forward-line -1)
	    (beginning-of-line)
	    (cond ((> i (save-excursion (end-of-line)
					(current-column))))
		  ((< i (current-indentation)))
		  ((progn (while (and (not (eolp))
				      (< (current-column) i))
			    (forward-char))
